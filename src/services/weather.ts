import type { LaunchSite, WeatherCondition, SiteForecast } from '../types/weather';
...
const checkWindDirectionMatch = (windDir: number, siteOrientation: string): boolean => {
    const orientationRanges: { [key: string]: [number, number][] } = {
        ...
        'SE': [[105, 165]],
        'S': [[165, 195]],
        'SSW': [[180, 225]],  // Added SSW specifically for Sylmar
        'SW': [[195, 255]],
        'W': [[255, 285]],
        'NW': [[285, 345]],
        'SW-W': [[195, 285]],
...
const determineSoaringFlyability = (
    _site: LaunchSite, // Fixed unused variable
    windSpeed: number,
    windGust: number,
...
const determineThermalFlyability = (
    ...
    // Relaxed wind direction constraint for thermalling - often light cross is ok.
    // But for now, strict.
    if (!windDirectionMatch) return 'poor';
...
const determineFlyability = (
    ...
    if (!windDirectionMatch) {
        ...
        flyability = 'poor';
        conditions = `Poor conditions: ${thermalStrength}/10 thermals`;
    }
...
export const getWeatherForecast = async (): Promise<SiteForecast[]> => {
    ...
    const targetDates = [
        getPacificDateString(0),
        getPacificDateString(1),
        getPacificDateString(2)
    ];

    const forecasts: SiteForecast[] = [];

    for (const site of launchSites) {
        const data = await fetchHRRRData(site);

        if (!data) {
            forecasts.push({
                site,
                forecast: targetDates.slice(0, 2).map(date => ({
                    date,
                    windSpeed: 0,
                    windDirection: 0,
                    windGust: 0,
        ...
        for (let i = 0; i < Math.min(2, targetDates.length); i++) {
            const targetDate = targetDates[i];
            const dayForecast = processHRRRDataForDay(site, data, targetDate);
            if (dayForecast) {
                ...
                forecastData.push({
                    ...
                    topOfLift: site.elevation,
                    flyability: 'poor' as const,
                    conditions: 'Data not available for this date',
                    soaringFlyability: 'poor' as const,
                    thermalFlyability: 'poor' as const,
                    launchTime: '12:00 PM',
                    cape: 0,
